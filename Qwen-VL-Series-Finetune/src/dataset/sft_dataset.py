...
class SupervisedDataset(Dataset):
    ...
    def __init__(
        ...
        # Filter out samples whose video file is missing (e.g. failed downloads).
        before = len(list_data_dict)
        list_data_dict = [
            item for item in list_data_dict
            if "video" not in item or os.path.exists(
                item["video"] if isinstance(item["video"], str)
                else item["video"][0]
            )
        ]
        after = len(list_data_dict)
        if before != after:
            print(f"[SupervisedDataset] Skipped {before - after} sample(s) with missing video files "
                  f"({after}/{before} remaining).", flush=True)
