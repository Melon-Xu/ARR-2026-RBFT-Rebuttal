        ...
        for start in range(0, input_ids.size(0), batch_size):
            ...
            if pixel_values_videos is not None:
                if video_grid_thw is not None:
                    # pixel_values_videos is (total_patches, ...) — all videos concatenated,
                    # NOT indexed by batch position.  Use video_grid_thw (T, H, W per video)
                    # to compute the correct patch-level start/end offsets, exactly as the
                    # image branch does with rows_per_image / cum_rows above.
                    patches_per_video = video_grid_thw.prod(dim=-1)  # T*H*W for each video
                    cum_vid_patches = torch.cat([
                        torch.zeros(1, dtype=torch.long, device=patches_per_video.device),
                        patches_per_video.cumsum(0)
                    ])
                    vid_patch_start = cum_vid_patches[start].item()
                    vid_patch_end   = cum_vid_patches[start + batch_size].item()
                    model_inputs["pixel_values_videos"] = pixel_values_videos[vid_patch_start:vid_patch_end]
