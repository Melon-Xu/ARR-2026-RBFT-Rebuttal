...
class Qwen2VLGRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        if processing_class is None:
            if "Qwen2.5-VL" in model_id or "Qwen2-VL" in model_id or "Aria" in model_id:
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if "video" in inputs[0]:
            n = len(prompt_completion_ids)
            prompt_inputs["pixel_values_videos"] = prompt_inputs["pixel_values_videos"].repeat(n, 1)
            prompt_inputs["video_grid_thw"] = prompt_inputs["video_grid_thw"].repeat(n, 1)
            # Qwen2.5-VL adds second_per_grid_ts (seconds per temporal grid cell per video).
            # Must be repeated to match the number of completions, otherwise get_rope_index
            # raises IndexError when accessing second_per_grid_ts[video_index] for index >= 1.
            if "second_per_grid_ts" in prompt_inputs:
                prompt_inputs["second_per_grid_ts"] = prompt_inputs["second_per_grid_ts"].repeat(n)
